...
const fetchUnreadEmails = async (isDeepSync = false) => {
  ...
  try {
    ...
    for (const item of messages) {
      ...
      const cleanBody = parsed.text.split(/On.*wrote:/i)[0].trim();
      const inReplyTo = parsed.headers.get('in-reply-to');
      const original = inReplyTo ? await SentMail.findOne({ messageId: inReplyTo }) : null;
      if (!original) continue;
      console.log(`Analyzing Reply for: ${original.subject}`);

      try {
        const aiRes = await axios.post(`${process.env.AI_SERVICE_URL}/analyze-email`, { emails: [cleanBody] });
